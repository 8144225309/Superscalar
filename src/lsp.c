...
int lsp_run_factory_creation(lsp_t *lsp,
    ...
    /* Generate LSP's own nonces (participant index 0) and store secnonces */
    size_t lsp_node_count = 0;
    for (size_t i = 0; i < f->n_nodes; i++) {
        if (factory_find_signer_slot(f, i, 0) >= 0)
            lsp_node_count++;
    }

    /* Allocate secnonces for LSP's nodes */
    secp256k1_musig_secnonce *lsp_secnonces =
        (secp256k1_musig_secnonce *)calloc(lsp_node_count,
                                            sizeof(secp256k1_musig_secnonce));
    if (!lsp_secnonces) return 0;
    ...
    if (!all_nonce_entries) { free(lsp_secnonces); return 0; }
    ...
    for (size_t i = 0; i < f->n_nodes; i++) {
        ...
        if (!musig_generate_nonce(lsp->ctx, &lsp_secnonces[secnonce_idx], &pubnonce,
                                   lsp_seckey, &lsp->lsp_pubkey,
                                   &f->nodes[i].keyagg.cache)) {
            fprintf(stderr, "LSP: nonce gen failed node %zu\n", i);
            goto fail;
        }

        if (!factory_session_set_nonce(f, i, (size_t)slot, &pubnonce)) {
        ...
        musig_pubnonce_serialize(lsp->ctx, nonce_ser, &pubnonce);
...
fail:
    ...
    memset(lsp_secnonces, 0, lsp_node_count * sizeof(secp256k1_musig_secnonce));
    free(lsp_secnonces);
    free(all_nonce_entries);
    return 0;
}
