...
/* ChaCha20 block function — RFC 7539 Section 2.3.2 test vector */
int test_chacha20_block_rfc7539(void) {
    /* Input state: "expand 32-byte k" + key + counter=1 + nonce */
    ...
    /* Expected output (after 20 rounds + add input) */
    ...
    TEST_ASSERT(memcmp(out, expected, 64) == 0, "ChaCha20 block output mismatch");

    return 1;
}

/* Poly1305 MAC — RFC 7539 Section 2.5.2 test vector */
int test_poly1305_rfc7539(void) {
    ...
    TEST_ASSERT_MEM_EQ(tag, expected_tag, 16, "Poly1305 tag mismatch");
